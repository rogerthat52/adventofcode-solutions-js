...
class Character {
    constructor(name, HP, damage, mana, armor){
        this.name = name;
        this.HP = HP;
        this.damage = !isNaN(damage) ? damage : 0;
        this.armor = !isNaN(armor) ? armor : 0;
        this.mana = !isNaN(mana) ? mana : 0;
        this.alive = true;
        this.cost = 0;
        this.items = [];
    }
...
function buildBoss(input){
    ...
    const Name = "Boss";
    const HP = parseInt(lineArray[0].split(" ").pop());
    const Damage = parseInt(lineArray[1].split(" ").pop());
    const Armor = parseInt(lineArray[2].split(" ").pop());
